...
let activeTaste = "food", currentStep = 0, quizIndex = 0, activeRoute = null, scores = { food: 0, view: 0, quiet: 0, photo: 0 };
const routeStops = {
...
function renderRoutes() { const todayRoutes = dailyRoutes(activeTaste); routeList.innerHTML = todayRoutes.map(route => `<button class="route-card" aria-label="${route.name} 길 안내 보기"><span class="route-art">${route.icon}</span><span class="route-info"><span class="route-top"><b>${route.name}</b><span class="route-crowd">${tasteNames[activeTaste]}</span></span><p>${route.detail}</p><span class="route-meta"><span>⌁ ${route.meta}</span></span></span><span class="arrow">→</span></button>`).join(""); document.querySelectorAll(".route-card").forEach((card, index) => card.addEventListener("click", () => openNavigation(todayRoutes[index]))); document.getElementById("routeHeading").textContent = `${tasteNames[activeTaste]} 오늘의 추천`; }
function updateNavigation() { const stops = routeStops[activeRoute.name]; const step = stops[currentStep + 1]; document.getElementById("modalTitle").textContent = activeRoute.name; document.getElementById("navPlace").textContent = step; document.getElementById("navDescription").innerHTML = `${currentStep === 1 ? "가상의 로컬 탐방 지점도 함께 둘러보세요." : "해안길을 따라 다음 탐방 지점으로 이동하세요."} <span class="demo-label inline">예시 안내</span>`; document.querySelectorAll(".map-stop").forEach((stop, index) => { stop.classList.toggle("current", index === currentStep + 1); stop.querySelector("small").textContent = stops[index]; }); document.getElementById("completeRoute").textContent = currentStep === stops.length - 2 ? "루트 완주하기" : "다음 지점으로 안내받기"; }
...
document.getElementById("startButton").addEventListener("click", showQuiz); document.getElementById("beginQuiz").addEventListener("click", beginQuiz); document.getElementById("restartQuiz").addEventListener("click", beginQuiz); document.getElementById("viewResultRoutes").addEventListener("click", () => { quiz.hidden = true; document.getElementById("routes").scrollIntoView({ behavior: "smooth" }); }); ["skipQuiz", "stopQuiz"].forEach(id => document.getElementById(id).addEventListener("click", () => { quiz.hidden = true; document.getElementById("routes").scrollIntoView({ behavior: "smooth" }); })); document.getElementById("homeButton").addEventListener("click", goHome); document.getElementById("navHome").addEventListener("click", goHome); document.getElementById("navRoute").addEventListener("click", () => document.getElementById("routes").scrollIntoView({ behavior: "smooth" })); document.getElementById("routeMakerButton").addEventListener("click", () => openNavigation()); document.getElementById("closeModal").addEventListener("click", closeModal); modal.addEventListener("click", event => { if (event.target === modal) closeModal(); }); document.getElementById("completeRoute").addEventListener("click", () => { const stopCount = routeStops[activeRoute.name].length; if (currentStep < stopCount - 2) { currentStep += 1; updateNavigation(); } else { showNotice(`${activeRoute.name} 루트를 완주했어요!`); closeModal(); } }); document.querySelectorAll(".map-stop").forEach((stop, index) => stop.addEventListener("click", () => { if (activeRoute && index > 0 && index < routeStops[activeRoute.name].length) { currentStep = index - 1; updateNavigation(); } })); document.querySelectorAll(".taste-tab").forEach(tab => tab.addEventListener("click", () => { activeTaste = tab.dataset.taste; document.querySelectorAll(".taste-tab").forEach(item => item.classList.toggle("active", item === tab)); renderRoutes(); })); document.getElementById("storyButton").addEventListener("click", () => showNotice("AR 도슨트는 다음 단계에서 연결할 기능이에요.")); renderRoutes();
